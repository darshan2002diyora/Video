  ...
  const generateCurrentScene = async () => {
    ...
    try {
      ...
      setResults((previous) => [
        ...
        { scene: scene.scene, title: scene.title, url: result?.url || result?.outputs?.[0] || null },
      ]);
      ...
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-[#0b0b0b] border border-white/10 rounded-3xl shadow-2xl">
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-white/5 rounded-2xl p-4"><div className="text-white/40 text-[10px] uppercase font-bold">Scenes</div><div className="text-white text-xl font-black mt-1">{scenes.length}</div></div>
            <div className="bg-white/5 rounded-2xl p-4"><div className="text-white/40 text-[10px] uppercase font-bold">Story length</div><div className="text-white text-xl font-black mt-1">{formatSeconds(totalDuration)}</div></div>
            ...
            <div className="bg-white/5 rounded-2xl p-4"><div className="text-white/40 text-[10px] uppercase font-bold">Model</div><div className="text-white text-sm font-black mt-2 truncate">{defaultModel.name}</div></div>
          </div>
          ...
          <div className="bg-[#d9ff00]/5 border border-[#d9ff00]/20 rounded-2xl p-4">
            ...
            <p className="text-white/50 text-xs mt-1">Generate one scene at a time first. This prevents accidentally starting a large paid generation job. Your current scene is {currentScene + 1} of {scenes.length}.</p>
          </div>
              ...
              return (
                <div key={scene.scene} className={`rounded-2xl border p-4 ${index === currentScene ? 'border-[#d9ff00]/40 bg-[#d9ff00]/5' : 'border-white/5 bg-white/[0.02]'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0"><div className="text-white text-sm font-bold truncate">{scene.title}</div><div className="text-white/30 text-xs">Planned duration: {scene.duration}s</div></div>
          ...
          {currentScene >= scenes.length && <p className="text-center text-[#d9ff00] font-bold text-sm">All story scenes have been generated.</p>}
        </div>
      </div>
    </div>
  );
